		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="需求时间" prop="xuqiushijian">
							<el-date-picker
								class="list_date"
								v-model="form.xuqiushijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.xuqiushijian?true:false"
								placeholder="请选择需求时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="任务状态" prop="renwuzhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.renwuzhuangtai?true:false"
								v-model="form.renwuzhuangtai" 
								placeholder="请选择任务状态"
								>
								<el-option v-for="(item,index) in renwuzhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='xuqiushijian'){
					form.value.xuqiushijian = row[x];
					disabledForm.value.xuqiushijian = true;
					continue;
				}
				if(x=='baochou'){
				...
				if(x=='renwuzhuangtai'){
					form.value.renwuzhuangtai = row[x];
					disabledForm.value.renwuzhuangtai = true;
					continue;
				}
				if(x=='fabushijian'){
		...
		renwuzhuangtaiLists.value = "已接单,待接单".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
