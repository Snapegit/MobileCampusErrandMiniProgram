...
<template>
	<div>
			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('paotuirenwu','接受')" type="success" @click="jieshourenwuCrossAddOrUpdateHandler(scope.row,'cross','','renwuzhuangtai','已接单','已接单,待接单'.split(',')[0])">接受</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<jieshourenwuFormModel ref="jieshourenwuFormModelRef" @formModelChange="formModelChange"></jieshourenwuFormModel>
	</div>
</template>
<script setup>
	...
	import jieshourenwuFormModel from '@/views/jieshourenwu/formModel'
	const jieshourenwuFormModelRef = ref(null)
    const jieshourenwuCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			jieshourenwuFormModelRef.value.init(row.id,'cross','接受',row,'paotuirenwu',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
